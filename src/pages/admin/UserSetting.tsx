...
import UserSettingTable from "../../components/admin/UserSettingTable";
...
const UserSetting: React.FC = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        console.log("getAllUsers response:", response); // Log response for debugging
  ...
  return (
    <div className="p-4">
      <RestaurantTitle text="User Settings" /> {/* Fixed typo */}
      {loading && <div className="text-center p-4 text-gray-600 dark:text-gray-300">Loading...</div>}
      {error && <div className="text-center p-4 text-red-500">{error}</div>}
      {!loading && !error && (
        <UserSettingTable headers={TABLE_HEADERS} data={users} setUsers={setUsers} />
      )}
    </div>
  );
};
